 ...
 * @var \App\Model\Entity\BpProcurement[]|\Cake\Collection\CollectionInterface $bpProcurements
 * @var array<int, \App\Model\Entity\BpContact> $procurementContactMap
...
?>
<?php
$this->assign('title', __('BP Procurements'));
$this->Breadcrumbs->add([
    ['title' => __('Home'), 'url' => '/'],
    ['title' => __('List BP Procurements')],
]);
?>
...
<div class="card card-primary card-outline">
  ...
  <div class="card-body border-bottom">
    ...
    <div class="cp-monthly-sales-status">
      <h3 class="cp-monthly-sales-title"><?= __('BP担当別 月次営業状況') ?></h3>
      ...
      <?php else: ?>
        ...
        <div class="cp-monthly-sales-tabs" role="tablist" aria-label="<?= __('BP担当別月次集計') ?>">
  ...
  <div class="card-body table-responsive p-0">
    <table class="table table-hover text-nowrap">
      ...
      <tbody>
        <?php if (empty($bpProcurements->toArray())): ?>
          <tr>
            <td colspan="7" class="text-muted"><?= __('No procurements found.') ?></td>
          </tr>
        <?php else: ?>
          <?php foreach ($bpProcurements as $procurement): ?>
            <?php
              $statusClass = 'cp-status-unselected';
              if ((int)$procurement->sales_status === BP_PROCUREMENT_STATUS_ENTRY) {
                  $statusClass = 'cp-status-interview';
              } elseif ((int)$procurement->sales_status === BP_PROCUREMENT_STATUS_DECLINED) {
                  $statusClass = 'cp-status-ng';
              } elseif ((int)$procurement->sales_status === BP_PROCUREMENT_STATUS_NO_REPLY) {
                  $statusClass = 'cp-status-no-reply';
              }
            ?>
            <tr class="<?= h($statusClass) ?>">
              <?php $procurementFormId = 'procurement-sales-form-' . (int)$procurement->id; ?>
              <?php $salesStatusValue = $procurement->sales_status !== null ? (int)$procurement->sales_status : BP_PROCUREMENT_STATUS_PROCURING; ?>
              <?php $salesReasonValue = $procurement->sales_reason !== null ? (int)$procurement->sales_reason : BP_PROCUREMENT_REASON_UNSET; ?>
              <td>
                <?= $this->Form->create(null, [
                    'url' => ['action' => 'updateSalesResult'],
                    'id' => $procurementFormId,
                    'class' => 'd-flex align-items-center',
                ]) ?>
                <?= $this->Form->hidden('procurement_id', ['value' => (int)$procurement->id]) ?>
              ...
              <td><?= h($procurement->received_at) ?></td>
              <td>
                <?php
                  $senderRaw = (string)$procurement->sender;
              ...
              <td>
                <?php if (!empty($procurementContactMap[$procurement->id])): ?>
                  <?php $contact = $procurementContactMap[$procurement->id]; ?>
                  <?php if ($contact->has('bp')): ?>
                    <?php
                      $bpNameRaw = str_replace(['株式会社', '合同会社'], '', (string)$contact->bp->name);
                      $bpNameSanitized = trim(preg_replace('/[\x00-\x1F\x7F]+/u', ' ', $bpNameRaw) ?? '');
                    ?>
                    <?= $this->Html->link(
                      h($this->Text->truncate($bpNameSanitized, 15)),
                        ['controller' => 'Bps', 'action' => 'view', $contact->bp->id]
                    , ['escape' => false]) ?>
                  <?php else: ?>
                    <?= __('Bp') ?>
                  <?php endif; ?>
                  ...
                  <?= $this->Html->link(
                      (string)$contact->name,
                      ['controller' => 'BpContacts', 'action' => 'view', $contact->id]
                  ) ?>
                <?php else: ?>
                  <?php
                    $rawRecipient = (string)$procurement->recipient;
              ...
              <td>
                <?php
                  $sanitizedSubject = trim(preg_replace('/[\x00-\x1F\x7F]+/u', ' ', (string)$procurement->subject) ?? '');
                ?>
                <?= $this->Html->link(
                  h($this->Text->truncate($sanitizedSubject, 40)),
                    ['action' => 'view', $procurement->id],
                    ['escape' => false]
                ) ?>
              </td>
            </tr>
