...
class Pop3ClientProposalService
{
        ...
        try {
            ...
            foreach ($uids as $uid) {
                ...
                $exists = (int)$this->runWithReconnect(function () use ($uidString): int {
                    return $this->clientProposals->find()
                ...
                $entity = $this->clientProposals->newEntity([
                    ...
                    'sales_status' => CLIENT_PROPOSAL_SALES_STATUS_PROPOSING,
                    'sales_reason' => CLIENT_PROPOSAL_REASON_UNSET,
                ...
                if ($this->runWithReconnect(function () use ($entity): bool {
                    return (bool)$this->clientProposals->save($entity);
                })) {
                    $imported++;
                }
            }
    ...
    private function refreshConnectionForSessionWrite(): void
    {
        $this->runWithReconnect(function (): void {
            $this->clientProposals->getConnection()->execute('SELECT 1');
        });
    }

    private function reconnectDatabase(): void
    {
        $connection = $this->clientProposals->getConnection();
